...
def parseHelper(folderDir, markfile):
  if 'Frank' in folderDir:
    for line in markfile.read().splitlines():
      if 'Total before late penalty: [' in line:
        string_contains_total_mark = line.split('[')[1].split('/')[0]
    ...
    if string_contains_total_mark == '':
      print('[WARNING][a8]' + folderDir+ ' no marks given!')
    mark = float(string_contains_total_mark) if string_contains_total_mark != '' else None

  if mark is None:
    print('[a8] WARNING: ' + folderDir + ' does not contain a valid mark')
  return mark
